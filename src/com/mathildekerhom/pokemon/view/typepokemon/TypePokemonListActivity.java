/**************************************************************************
 * TypePokemonListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display TypePokemon entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a TypePokemon list. A click on an item will get 
 * you to TypePokemonShowActivity.
 * - On a tablet, the TypePokemonShowFragment will be displayed right next to
 * the list.
 */
public class TypePokemonListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected TypePokemonListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected TypePokemonShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (TypePokemonShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (TypePokemonListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_typepokemon_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, TypePokemonShowActivity.class);
            final TypePokemon item = TypePokemonContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(TypePokemonContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(TypePokemon item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            TypePokemon item = TypePokemonContract.cursorToItem(
